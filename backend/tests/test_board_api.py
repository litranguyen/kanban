...
def test_get_board_returns_seed_board(tmp_path: Path) -> None:
    ...
    assert payload["columns"][1]["id"] == SEED_BOARD["columns"][1]["id"]
...
def test_put_board_updates_and_persists(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        board["columns"][1]["title"] = "Ready Revised"
        board["columns"][1]["cards"][0]["title"] = "Updated onboarding copy"
        ...
        assert updated_board["columns"][1]["title"] == "Ready Revised"
        assert updated_board["columns"][1]["cards"][0]["title"] == "Updated onboarding copy"
        ...
        assert reloaded_board["columns"][1]["title"] == "Ready Revised"
        assert reloaded_board["columns"][1]["cards"][0]["title"] == "Updated onboarding copy"
